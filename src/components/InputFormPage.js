...
const handleArrayChange = (e, index, field) => {
  ...
  const handleAdditionalImageChange = (e) => {
    ...
    if (file) {
      if (file.size > 500 * 1024) { // 500KB size limit
        alert('The selected image exceeds the 500KB limit. Please choose a smaller file.');
        e.target.value = null; // Clear the file input
        return;
      }
  
      // Set the additional image if the size is within the limit
  ...
  const handleSubmit = (e) => {
    ...
    const requiredFields = Object.entries(formData)
      .filter(([key]) => !["चूलते", "दाजी", "भाऊ", "बहीण", "मामा", "गण" ,"गोत्र"].includes(key))
      .map(([key, value]) => {
        if (value.type === "date") {
          return {
            key,
